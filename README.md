...
(1)、新建一个工程，在`main.m`中新增`TestClass`类
...
运行一下代码，可以从打印的结果上看出类方法和实例方法的区别：**实例方法打印出了`TestClass`这个对象的指针，类方法打印出来就是`TestClass`这个类**。
...
一般情况下，如果调用一个方法,其本质是利用`objc_msgSend`方法发送一调消息，代码如下：
...
此时，需要导入系统提供的`#import <objc/message.h>`
  ![](https://upload-images.jianshu.io/upload_images/1214383-57cc529e7572a64f.png?imageMogr2/auto-orient/strip%7CimageView2/2/w/1240)
```
  TestClass *test = [[TestClass alloc]init];
